...
; Get line address from Y
	mov eax, dword [ ebp + 12 ]	; Set EAX with parameter 2 (4*(1+2))
...
; Get line address from X
	mov eax, dword [ ebp + 8 ]	; Set EAX with parameter 1 (4*(1+1))
	...
	mov esi, dword [ ebp + 16 ]	; parameter 3 (4*(1+3))
...
.MESSAGELOOP:                           ; Print message
        ...
        mov byte [ edi + 0xB8000], cl   ; if cl is not 0, print value of edi + 0xB8000 address
...
times 512 - ( $ - $$ ) db 0x00		; Fill 512 bytes
